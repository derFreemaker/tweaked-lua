...
static void hookf(lua_State* L, lua_Debug* ar)
{
    ...
    if (lua_rawget(L, -2) == LUA_TFUNCTION)
    {
        ...
        lua_call(L, 3, 0); /* call hook function */
    }
}
...
static int call_external_hook_from_lua(lua_State* L)
{
    lua_Hook hook = lua_touserdata(L, lua_upvalueindex(1));
    lua_Debug* ar = lua_touserdata(L, 3);
    hook(L, ar);
    return 0;
}
...
static int db_gethook(lua_State* L)
{
    ...
    else if (hook != hookf)
    {
        ...
        lua_pushcclosure(L, call_external_hook_from_lua, 1);
    }
    else
    {
